    ...
    <div className="flex flex-col gap-6" style={{ boxSizing: 'border-box' }}>
           ...
           <div className="flex gap-2 p-1 bg-[#1e2336]/60 backdrop-blur-md rounded-2xl border border-white/5 relative z-10 shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
              {['Global'].map(t => (
                 <button
                    ...
                    className={`px-10 py-2.5 rounded-xl text-xs font-bold transition-all relative ${activeTab === t ? 'text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                 >
                    {activeTab === t && (
       ...
       <div className="p-4 mt-auto bg-gradient-to-t from-[#0a0f1c] to-transparent pt-6 relative z-10">
          <div className="w-full flex justify-center py-2">
            <button 
               onClick={handleShare}
               className="w-[70%] max-w-[280px] py-4 px-6 rounded-3xl text-white text-sm font-black uppercase tracking-wider transition-all shadow-[0_0_20px_rgba(168,85,247,0.5)] hover:shadow-[0_0_30px_rgba(168,85,247,0.7)] active:scale-95 flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-fuchsia-500 whitespace-nowrap"
            >
               {copied ? 'NUSXALANDI!' : 'SHARE RANK'} <Trophy size={18} className="drop-shadow-md" />
            </button>
          </div>
       </div>
