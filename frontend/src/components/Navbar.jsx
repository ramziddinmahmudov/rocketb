...
export default function Navbar({ currentTab, setTab }) {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-40" style={{ marginBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      <nav className="bg-[#050A18]/90 backdrop-blur-xl border border-white/10 h-[64px] flex items-center rounded-3xl px-[20px] shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
         ...
         {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center w-14 h-[50px] gap-1 relative transition-all ${isActive ? 'text-white' : 'text-gray-500 hover:text-gray-400'}`}
              >
                {isActive && (
                   <motion.div
                     layoutId="nav-pill"
